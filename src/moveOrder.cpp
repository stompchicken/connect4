...
void MoveOrder::reorder(Depth depth, Move cachedMove, Move* moves) const {
    ...
    assert(depth >= 0 && depth <= DEPTH_MAX);
#endif
    if(useStaticMove && !useKillerMove && !useBestMove) {
        return;
    } else if(useStaticMove && !useKillerMove && !useBestMove) {
        getStaticOrder(moves);
    } else {
        uint64 value[WIDTH];
        for(Move i=0; i<WIDTH; i++) {
            ...
            if(useKillerMove && i == killerMove[depth]) { value[i] = 10; }
            // Give cached moves highest priority
            if(useBestMove && i == cachedMove) { value[i] = 11; }
        }
