...
bool operator==(const Entry& lhs, const Entry& rhs) {
    return lhs.value == rhs.value && lhs.depth == rhs.depth && lhs.bestMove == rhs.bestMove;
}

std::ostream& operator<<(std::ostream &output, const Entry &val) {
    output << "{" << printValue(val.value);
...
void packEntry(const Key& key, const Entry& entry, Packed& packed) {
    ...
    packed |= static_cast<uint64_t>(entry.value) << 13;
    packed |= static_cast<uint64_t>(entry.depth) << 7;
    packed |= static_cast<uint64_t>(entry.bestMove) << 3;
}

void unpackEntry(const Packed& value, Key& key, Entry& entry) {
    key = static_cast<uint64_t>(value >> 15);
    entry.value = static_cast<uint8_t>(value >> 13) & 0x3;
    entry.depth = static_cast<uint8_t>(value >> 7) & 0x3f;
    entry.bestMove = static_cast<uint8_t>(value >> 3) & 0x7;
}
...
//        std::cout << "PUT: index=" << index << " key=" << key << " " << currentEntry << " -> " << entry;
        if(currentKey == 0) {
            // Insert new entry
